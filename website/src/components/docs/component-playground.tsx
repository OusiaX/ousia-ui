...
export const ComponentPlayground = async () => {
  ...
  const programm = pipe(
    Effect.succeed(path.join(process.cwd(), `../components/src/demos/${component}.demo.tsx`)),
    Effect.flatMap((demoFile) =>
      Effect.tryPromise({
        try: () => readFile(demoFile, 'utf8'),
        catch: () => new Error(`Demo file not found: ${component}`),
      }),
    ),
    Effect.flatMap((code) =>
      Effect.promise(() => highlight(code, 'tsx')).pipe(
        Effect.map((html) => ({
          code,
          html,
        })),
